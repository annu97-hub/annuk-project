...
exports.saveUser = (userObj) => {
  return new Promise(async (resolve, reject) => {
    ...
    // mail func
    Util.EmailUtil.sendMail(userObj.email, "New OTP generated", "otp-verify", {
      otp: userObj.otpCode,
    });
    resolve(saveUser);
  });
...
exports.saveJob = (jobObj) => {
  return new Promise(async (resolve, reject) => {
    let jobModel = new Model.Job(jobObj);

    let saveUser = await jobModel.save();

    resolve(saveUser);
  });
};

exports.listJob = (jobObj) => {
  return new Promise(async (resolve, reject) => {
    let jobList = await Model.Job.aggregate([
      //        { $match: { status: 1 } },
      { $sort: { createdAt: -1 } },
    ]);
...
exports.sendOtp = (requestParams) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (user == null) {
      ...
      // mail func
      Util.EmailUtil.sendMail(email, "New OTP generated", "otp-verify", {
        otp: otpCode,
      });
